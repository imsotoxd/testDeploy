...
const LastAdded = async () => {
   ...
   const stockClass = (quantity: number, minimumQuantity: number) => clsx("font-semibold rounded text-xs w-1/2 h-fit p-1 inline-block text-center", {
    "text-red-800 bg-red-200": quantity === 0,
    "text-yellow-800 bg-yellow-200": quantity <= minimumQuantity && quantity > 0,
   ...
   return(
       <div className="mx-10">
           ...
           {products.map((product) => (
               <div key={product.id} className="grid grid-cols-6 p-2 border-b items-center">
                   <span className="col-span-2 truncate">{product.name}</span>
                   <span className="col-span-2 truncate">{product.Category.name}</span>
                   <span className="text-center">{product.quantity}</span>
                   <span className="text-center">
                       <span className={stockClass(product.quantity, product.minimumQuantity)}>
           ...
           <button className="bg-primary px-4 mt-2  py-1 rounded-xl text-white">Ver mas</button>
           </Link>
           </div>
       </div>   
   )
